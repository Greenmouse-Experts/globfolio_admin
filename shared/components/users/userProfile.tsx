...
import { useGetOneUsersQuery, useLazySuspendUserQuery, useLazyUnsuspendUserQuery } from "@/services/api/userSlice";
    ...
    <>
      <div>
        {isLoading && "Loading..."}
        {
          ...
          <div className="mt-8">
            {
              data?.message.isSuspended?
              <Button
              title={isBusy ? <ScaleSpinner size={14} color="white"/> : "Unsuspend User"}
              altClassName="w-full rounded-lg py-3 btn-like"
              onClick={() => unsuspendUser(item.id)}
            />
            : 
            <Button
            title={isBusy ? <ScaleSpinner size={14} color="white"/> : "Suspend User"}
